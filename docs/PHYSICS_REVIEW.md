...
| P1 | The stall rule is a fixed 35% idle-RPM threshold. | It is stable and useful, but it is not derived from combustion stability or idle actuator limits. | Replace with an idle-controller torque reserve and measured stall map after engine data is available. |
